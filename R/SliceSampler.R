...
bsgw.convert.multivar.to.univar <- function(xk, k, x, f, ...) {
  x[k] <- xk
  return (f(x, ...))
}
# Gibbs wrapper around univariate slice sampler to convert it to a multivariate sampler
bsgw.multislice.from.unislice <- function(xm, fm, ..., w=1.0, m=Inf, lower=rep(-Inf,length(xm)), upper=rep(+Inf,length(xm))) {
  K <- length(xm)
  for (k in 1:K) {
    #cat("k=",k,", xm[k]=", xm[k], "\n")
    xm[k] <- bsgw.uni.slice(xm[k], bsgw.convert.multivar.to.univar, k, xm, fm, ..., w=w, m=m, lower=lower[k], upper=upper[k])
  }
  return (xm)
}
...
bsgw.uni.slice <- function (x0, f, ..., w=1, m=0, lower=-Inf, upper=+Inf, gx0=NULL)
{
  g <- function(x) f(x,...)
